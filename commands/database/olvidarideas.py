...
SARCASTIC_MESSAGES = [
    ...
    "Ups… parece que quieres que guarde todas sus ideas de tus viewersotra vez 🙄",
...
class OlvidarIdeas(commands.Cog):
    ...
    def __init__(self, bot, memoria):
        ...
        self.confirmando = set()  # mods/broadcaster que están confirmando
    ...
    async def borrar_ideas(self, ctx: commands.Context, *args):
        ...
        if user not in self.confirmando or not args:
            ...
            await ctx.send(f"@{ctx.author.name} ⚠️ Esto borrará **todas las ideas del sorteo**. "
